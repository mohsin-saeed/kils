...
<div style="background: url('<?php echo $page[0]->bgUrl?>'); background-repeat: no-repeat; width: <?php echo $PAGE_WIDTH?>; height: <?php echo $PAGE_HEIGHT?>; ">
    ...
    <?php foreach($raw_data as $obj){
        ...
        ?>
            <div style=" background: url('{{url("storage/public/objectsbg/".$obj->states[0]->bg)}}'); background-repeat: no-repeat; background-size:{{$width}}px {{$height}}px;  position: fixed; top:{{$y}}; left: {{$y}}; width:{{$width}}px; height: {{$height}}px" id="obj-{{$obj->id}}">
